...
qui foreach index in SPY VIX VIXY LQD_LQDH GOVT LQD IVR GOVT_TIP GLD JPY EUR {
	...
	*							plot results
	*===============================================================================
qui foreach index in SPY VIX VIXY LQD_LQDH GOVT IVR GOVT_TIP GLD JPY EUR {
	...
	twoway (line bs_b_emo hh if hh<=`mm', lpattern(solid) lcolor(blue)  lwidth(thick)) ///
			...
			, ///
			legend(off) ///
			yti("Basis points", si(small)) yline(0) ti("`index': Voice tone", size(medium)) name(`index'_emo, replace) ///
	...
	twoway (line bs_b_emoNC hh if hh<=`mm', lpattern(solid) lcolor(midblue)  lwidth(thick)) ///
			...
			, ///
			legend(off) ///
			yti("Basis points", si(small)) yline(0) ti("`index': Voice tone (no controls)", size(medium)) name(`index'_emoNC, replace) ///
			xtitle("Days from FOMC press conference", size(small)) xlab(, labs(small)) nodraw
	...
	twoway (line bs_b_S_QASR0 hh if hh<=`mm', lpattern(solid) lcolor(red)  lwidth(thick)) ///
			...
			, ///
			legend(off) ///
			yti("Basis points", si(small)) yline(0) ti("`index': Text sentiment", size(medium)) name(`index'_senti, replace) ///
			xtitle("Days from FOMC press conference", size(small)) xlab(, labs(small)) nodraw
	...
	twoway (line bs_b_FFR hh if hh<=`mm', lpattern(solid) lcolor(black)  lwidth(thick)) ///
			...
			, ///
			legend(off) ///
			yti("Basis points", si(small)) yline(0) ti("`index': FFR shock", size(medium)) name(`index'_FFR, replace) ///
			xtitle("Days from FOMC press conference", size(small)) xlab(, labs(small)) nodraw
	...
	twoway (line bs_b_FG hh if hh<=`mm', lpattern(solid) lcolor(green)  lwidth(thick)) ///
			...
			, ///
			legend(off) ///
			yti("Basis points", si(small)) yline(0) ti("`index': FG shock", size(medium)) name(`index'_FG, replace) ///
			xtitle("Days from FOMC press conference", size(small)) xlab(, labs(small)) nodraw
	...
	twoway (line bs_b_AP hh if hh<=`mm', lpattern(solid) lcolor(gold)  lwidth(thick)) ///
			...
			, ///
			legend(off) ///
			yti("Basis points", si(small)) yline(0) ti("`index': AP shock", size(medium)) name(`index'_AP, replace) ///
			xtitle("Days from FOMC press conference", size(small)) xlab(, labs(small)) nodraw		
	
	if "`index'"=="SPY" loc vv=3
	if "`index'"=="VIX" loc vv=4
	if "`index'"=="VIXY" loc vv=5 
	if "`index'"=="LQD_LQDH" loc vv=6
	if "`index'"=="GOVT" loc vv=7
	if "`index'"=="LQD" loc vv=8
	if "`index'"=="IVR" loc vv=9 
	if "`index'"=="GOVT_TIP" loc vv=10
	if "`index'"=="GLD" loc vv=11
	if "`index'"=="JPY" loc vv=12
	if "`index'"=="EUR" loc vv=13          
	
	graph combine `index'_emoNC `index'_emo `index'_senti `index'_FFR `index'_FG `index'_AP, rows(2) imargin(tiny)
	gr export Fig`vv'.png, width(1800) replace
...
twoway (line bs_b_emo hh if hh<=`mm', lpattern(solid) lcolor(blue)  lwidth(thick)) ///
	...
	, ///
	legend(off) ///
	yti("Basis points", si(small)) yline(0) ti("LQD: Voice tone", size(medium)) name(LQD_emo, replace) ///
	xtitle("Days from FOMC press conference", size(small)) xlab(, labs(small)) nodraw
...
twoway (line bs_b_emoNC hh if hh<=`mm', lpattern(solid) lcolor(midblue)  lwidth(thick)) ///
	...
	, ///
	legend(off) ///
	yti("Basis points", si(small)) yline(0) ti("LQD: Voice tone (no controls)", size(medium)) name(LQD_emoNC, replace) ///
	xtitle("Days from FOMC press conference", size(small)) xlab(, labs(small)) nodraw
...
twoway (line bs_b_S_QASR0 hh if hh<=`mm', lpattern(solid) lcolor(red)  lwidth(thick)) ///
	...
	, ///
	legend(off) ///
	yti("Basis points", si(small)) yline(0) ti("LQD: Text sentiment", size(medium)) name(LQD_senti, replace) ///
	xtitle("Days from FOMC press conference", size(small)) xlab(, labs(small)) nodraw
...
twoway (line bs_b_FFR hh if hh<=`mm', lpattern(solid) lcolor(black)  lwidth(thick)) ///
	...
	, ///
	legend(off) ///
	yti("Basis points", si(small)) yline(0) ti("LQD: FFR shock", size(medium)) name(LQD_FFR, replace) ///
	xtitle("Days from FOMC press conference", size(small)) xlab(, labs(small)) nodraw
...
twoway (line bs_b_FG hh if hh<=`mm', lpattern(solid) lcolor(green)  lwidth(thick)) ///
	...
	, ///
	legend(off) ///
	yti("Basis points", si(small)) yline(0) ti("LQD: FG shock", size(medium)) name(LQD_FG, replace) ///
	xtitle("Days from FOMC press conference", size(small)) ylab(-60(10)0, labs(small)) xlab(, labs(small)) nodraw
...
twoway (line bs_b_AP hh if hh<=`mm', lpattern(solid) lcolor(gold)  lwidth(thick)) ///
	...
	, ///
	legend(off) ///
	yti("Basis points", si(small)) yline(0) ti("LQD: AP shock", size(medium)) name(LQD_AP, replace) ///
	xtitle("Days from FOMC press conference", size(small)) xlab(, labs(small)) nodraw		

       
graph combine LQD_emoNC LQD_emo LQD_senti LQD_FFR LQD_FG LQD_AP, rows(2) imargin(tiny)
gr export Fig8.png, width(1800) replace
erase LQD_temp.dta
